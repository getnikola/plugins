...
regex_code = re.compile('(' + META + '|' + DISABLED_META +
                        r'|````)|(``(?P<t>.+?)``(?::(?P<c>[a-zA-Z][_a-zA-Z\-\d]*)(?:\[(?P<p>[^\]]*)\])?)?)', re.S)
...
regex_link_level2 = re.compile(
    r'^(?P<t>\S.*?)?(?:\s+\[(?P<a>.+?)\])?(?:\s+(?P<k>\S+))?(?:\s+(?P<p>popup))?\s*$', re.S)
regex_media_level2 = re.compile(
    r'^(?P<t>\S.*?)?(?:\s+\[(?P<a>.+?)\])?(?:\s+(?P<k>\S+))?\s+(?P<p>img|IMG|left|right|center|video|audio|blockleft|blockright)(?:\s+(?P<w>\d+px))?\s*$', re.S)
...
def render(text,
            ...
            if c0 in "#+-.":  # first character is one of: # + - .
                ...
                if t1 or t2:
                    ...
                    if c0 == '#':  # headers
                        ...
                        (lev, mtag, lineno) = parse_point(
                            t2, ss, lev, mtag, lineno)
                        lineno += 1
                        continue
    ...
    def sub_media(m):
        ...
        title = ' title="%s"' % escape(a).replace(
            META, DISABLED_META) if a else ''
        p_begin = p_end = ''
        if p == 'center':
        ...
        try:
            ...
            if len(sys.argv) > 2:
                if sys.argv[2].startswith('@'):
                    markmin_style = '<link rel="stylesheet" href="' + \
                        sys.argv[2][1:] + '"/>'
                else:
