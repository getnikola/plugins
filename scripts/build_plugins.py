...
def build_plugin(plugin=None):
    ...
    if os.path.isdir('plugins/'+plugin):
        with cd('plugins/'):
            subprocess.check_call('zip -r ../output/v6/{0}.zip {0}'.format(plugin), stdout=subprocess.PIPE, shell=True)
    ...
    with open(os.path.join("output", "v6", "plugins.json"), "wb+") as outf:
        json.dump(plugins_dict, outf, indent=4, ensure_ascii=True, sort_keys=True)
